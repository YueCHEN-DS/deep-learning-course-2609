"""
Handwritten Digit Recognition - PyTorch MLP (Multi-Layer Perceptron) Training Pipeline
Architecture: 784 -> 256 (ReLU) -> 64 (ReLU) -> 10 (Softmax)
Trained on MNIST with data augmentation.
"""
...
class DigitMLP(nn.Module):
    """
    Multi-Layer Perceptron (Fully Connected Deep Feed-Forward Network).
    Total Parameters: ~218,000
    Unlike CNN, the MLP treats each of the 784 pixels independently without 2D spatial filters.
    """
    def __init__(self):
        super().__init__()
...
def export_weights_to_js(model, output_js_path):
    ...
    header = "/** Pre-trained MNIST MLP Weights (Trained with PyTorch) **/\n"
    content = header + "window.MLP_WEIGHTS = " + json.dumps(export_dict) + ";\n"
    with open(output_js_path, "w") as f:
        f.write(content)
    print(f"[*] Exported MLP weights to: {output_js_path}")


def train_mlp(epochs=5, batch_size=128, lr=0.002, data_dir="./data"):
    device = get_device()
    ...
    print(f"\n[*] Starting MLP training for {epochs} epochs...")
    ...
    print(f"\n[✓] MLP Training completed in {elapsed:.1f}s! Final Accuracy: {history['test_accuracy'][-1]}%")
    ...
    export_weights_to_js(model, "./mlp_weights.js")
